...
## 2026-07-02 - LLM Markdown Prompt Accessibility

**Learning:** When instructing LLMs to generate UI components (like HTML cards), generic ARIA attributes are insufficient; structural elements like `<dl>`, `<dt>`, `<dd>` and context-dependent roles like `role="status"` or `role="alert"` must be explicitly mandated in the markdown prompts.
**Action:** Update the accessibility guidelines in SKILL.md to explicitly mandate these semantic tags and ARIA roles for the generated components.
