...
## 2026-06-25 - Use semantic list elements and live region roles in LLM UI generation

**Learning:** When using an LLM to generate UI components like data rows or status bars, failing to explicitly specify semantic tags (like `<dl>`, `<dt>`, `<dd>`) and live region roles (`role="status"`, `role="alert"`) results in inaccessible HTML structures.
**Action:** Always instruct the LLM to use semantic HTML and correct live region roles for dynamically updated status indicators and key-value pairs when generating UI components.
