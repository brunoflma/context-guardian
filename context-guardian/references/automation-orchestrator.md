...
def needs_evacuation(state: ConversationState) -> bool:
    """Verifica se é necessário evacuar com base nos tokens (O(1) lookup)."""
...
def generate_evacuation_report(state: ConversationState) -> str:
    ...
    print(f"   Tokens no contexto: {state.current_context_tokens:,}")
...
def main():
    ...
    while True:
        ...
        if user_input.lower() == "status":
            pct = (state.current_context_tokens / CONTEXT_LIMIT) * 100
            print(f"\n📊 Status do contexto:")
            print(f"   Tokens no contexto: {state.current_context_tokens:,} / {CONTEXT_LIMIT:,} ({pct:.1f}%)")
...
async function main() {
  ...
  while (true) {
    ...
    if (input.toLowerCase() === "status") {
      const tokens = state.currentContextTokens;
      console.log(
        `\n📊 Tokens: ${tokens.toLocaleString()} / ${CONTEXT_LIMIT.toLocaleString()} (${((tokens / CONTEXT_LIMIT) * 100).toFixed(1)}%)`,
      );
