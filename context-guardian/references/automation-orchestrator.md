...
class ConversationState:
    ...
    current_context_tokens: int = 0 # ⚡ Bolt: Usando contagem exata da API em O(1) em vez de estimativa em O(N)
...
interface SessionState {
  ...
  currentContextTokens: number; // ⚡ Bolt: Usando contagem exata da API em O(1) em vez de estimativa em O(N)
}
