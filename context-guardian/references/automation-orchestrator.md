...
def generate_evacuation_report(state: ConversationState) -> str:
    ...
    try:
        ...
        print("❌  Erro na API ao gerar relatório (falha de comunicação segura).")
        return "Erro ao gerar relatório. Operação cancelada de forma segura."
...
def transfer_session(state: ConversationState) -> ConversationState:
    ...
    try:
        ...
        print("\n❌  Erro na API ao confirmar nova sessão (falha de comunicação segura).")
        return state
...
def chat(state: ConversationState, user_message: str) -> tuple[str, ConversationState]:
    ...
    try:
        ...
        error_msg = "Erro na comunicação com a API. A operação falhou de forma segura."
...
async function generateEvacuationReport(state: SessionState): Promise<string> {
  ...
  try {
    ...
    console.error("❌  Erro na API ao gerar relatório (falha de comunicação segura).");
    return "Erro ao gerar relatório. Operação cancelada de forma segura.";
  }
}
...
async function chat(
  ...
  try {
    ...
    const errorMsg = "Erro na comunicação com a API. A operação falhou de forma segura.";
    return [errorMsg, currentState];
  }
}
