...
def generate_evacuation_report(state: ConversationState) -> str:
    ...
    try:
        ...
        raise Exception("Erro na geração do relatório.")
...
def transfer_session(state: ConversationState) -> ConversationState:
    ...
    try:
        ...
        print("Transferência abortada devido a erro na geração do relatório.")
        state.transfer_count -= 1
        return state
...
async function transferSession(state: SessionState): Promise<SessionState> {
  ...
  let report: string;
  try {
    report = await generateEvacuationReport(state);
  } catch (error) {
    console.error("Transferência abortada devido a erro na geração do relatório.");
    return state;
  }
