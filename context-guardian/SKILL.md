...
- 🔒 **SEGURANÇA (Path Traversal):** O `[slug]` DEVE ser estritamente higienizado. Remova todos os pontos, barras, barras invertidas e caracteres especiais. Use apenas letras minúsculas, números e hifens. Nunca confie na entrada do usuário sem sanitização para formar o nome do arquivo.
...
- 🔒 **SEGURANÇA (XSS):** Todos os dados do usuário e conteúdos da conversa impressos em Cards HTML (ex: "Objetivo", "Decisões") DEVEM ter seus caracteres HTML especiais "escapados" (ex: `<` para `&lt;`, `>` para `&gt;`) para prevenir injeção de Cross-Site Scripting (XSS).
- 🔒 **SEGURANÇA (Path Traversal):** Como reforçado no Passo 3, nunca aceite caminhos arbitrários ou caracteres especiais no `slug` do arquivo .md, para evitar vulnerabilidades de Path Traversal.
