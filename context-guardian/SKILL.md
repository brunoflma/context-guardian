...
O card tem quatro zonas e **deve ser totalmente acessível** (incluir `role="status"`, e em SVGs decorativos usar `aria-hidden="true"`):
...
**Acessibilidade:** adicionar `role="progressbar"`, `aria-label="Progresso até o próximo checkpoint"`, `aria-valuenow="[turno atual]"`, `aria-valuemin="0"`, `aria-valuemax="[intervalo definido]"`.
