...
**Obrigatório para Acessibilidade:** O container principal do card deve ter `role="region"` e `aria-label="Status do Context Guardian"`. Todos os ícones SVG devem ter `aria-hidden="true"`.
...
A barra de progresso **deve** incluir `role="progressbar"`, `aria-valuenow="[turno_atual]"`, `aria-valuemin="0"`, e `aria-valuemax="[intervalo]"`.
