...
O HTML gerado DEVE conter os seguintes atributos de acessibilidade:

- O card principal deve ter `role="region"` e `aria-label="Status do Context Guardian"`.
- A barra de progresso (Zona 2) deve ter `role="progressbar"`, `aria-valuenow` (turno atual), `aria-valuemin="0"` e `aria-valuemax` (próximo checkpoint).
- Todos os ícones SVG (Zona 3) devem ter `aria-hidden="true"`.
- A barra de recomendação (Zona 4) deve ter `aria-live="polite"` para que leitores de tela anunciem as mudanças de estado.
