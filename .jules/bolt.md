...
## 2026-06-24 - [String partitioning in large contexts]
**Learning:** Using `str.partition()` on massive strings (like 200k+ token reports) allocates significant memory for the unused prefix before the marker.
**Action:** Use `str.find()` combined with string slicing (`string[index+len(marker):]`) when only the text after a marker is needed, avoiding unnecessary prefix allocation.
