...
## 2026-07-01 - Replace partition with find for large strings

**Learning:** In Python, when extracting a suffix after a marker from a very large string, prefer using str.find() with string slicing instead of str.partition() to avoid the overhead of allocating memory for an unused, massive prefix string.
**Action:** Always use str.find() with string slicing instead of str.partition() to perform a single pass and extract only the needed part without unnecessary allocations.
